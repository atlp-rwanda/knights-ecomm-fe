...
interface PopupProps {
  ...
  duration: number; // Duration to display the popup in milliseconds
  onClose: () => void;
}
...
const Popup: React.FC<PopupProps> = ({ title, subtitle, responseType, duration, onClose }) => {
  ...
  return (
    ...
    >
      <div
        ref={popupRef}
        className={`bg-white p-6 rounded-lg shadow-lg transform transition-transform duration-500 ease-in-out w-[542px] h-[351px] ${
      ...
      >
        ...
        />
        <h2 className="text-2xl font-bold mb-2 text-center text-custom-primary">{title}</h2>
        <h3 className="text-lg mb-4 text-center text-custom-secondary">{subtitle}</h3>
      </div>
    </div>
  );
};
