...
const CouponForm: React.FC<CouponFormProps> = ({ onSubmit, product }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      !formData.maxUsageLimit ||
      isNaN(Number(formData.maxUsageLimit)) ||
      Number(formData.maxUsageLimit) < 0
    ) {
  ...
  const handleSubmit = (event: FormEvent): void => {
    ...
    if (validateForm()) {
      const data: FormPayload = {
        ...
        maxUsageLimit: parseInt(formData.maxUsageLimit, 10),
        product
      };
      onSubmit(data);
      clearForm();
  ...
  return (
    <form onSubmit={handleSubmit} className="w-[542px]">
      <div className="flex gap-2 flex-col items-start">
        ...
        <input
          ...
          className={`text-sm outline-none w-full p-2 h-9 border ${errors.code ? 'border-red-500' : 'border-gray-300'} rounded bg-transparent`}
        />
      ...
      <div className="flex justify-between gap-4 mt-2">
        ...
        <div className="flex gap-2 flex-col items-start w-4/5">
          ...
          <input
            ...
            onChange={handleInputChange}
            className={`text-sm outline-none w-full p-2 h-9 border ${errors.discountRate ? 'border-red-500' : 'border-gray-300'} rounded bg-transparent`}
            placeholder={discountType === 'percentage' ? '20%' : '20 RWF'}
          />
          {errors.discountRate && (
            <span className="text-red-500 text-xs">{errors.discountRate}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-col items-start my-2">
        ...
        <input
          ...
          onChange={handleInputChange}
          className={`text-sm outline-none w-full p-2 h-9 border ${errors.expirationDate ? 'border-red-500' : 'border-gray-300'} rounded bg-transparent flex items-center justify-center`}
        />
        {errors.expirationDate && (
          <span className="text-red-500 text-xs">{errors.expirationDate}</span>
        )}
      </div>
      <div className="flex gap-2 flex-col items-start my-2">
        ...
        <input
          ...
          onChange={handleInputChange}
          className={`text-sm outline-none w-full p-2 h-9 border ${errors.maxUsageLimit ? 'border-red-500' : 'border-gray-300'} rounded bg-transparent`}
          min="0"
        />
        {errors.maxUsageLimit && (
          <span className="text-red-500 text-xs">{errors.maxUsageLimit}</span>
        )}
      </div>
    </form>
  );
